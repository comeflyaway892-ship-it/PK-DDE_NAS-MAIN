...
def d3pm_step(xt, x0, q, qbar, t, eps=1e-20):
    ...
    # 取累计到 t-1 的转移矩阵 qbar_{t-1}: [K, K]
    qbar_tm1 = qbar[t-1]  # q(x_{t-1} | x0)

    # 对每个位置：likelihood[k] = q_t[k, xt]  (取列 xt)
    # q_t[:, xt_flat] -> [K, N]  -> 转置成 [N, K]
    like = q_t[:, xt_flat].transpose(0, 1).contiguous()  # [N, K]

    # prior[k] = qbar_{t-1}[x0, k] (取行 x0)
    prior = qbar_tm1[x0_flat]  # [N, K]

    # 后验未归一化：post_unnorm[n,k] = like[n,k] * prior[n,k]A
...
class BayesianEstimator:
    ...
    def estimate(self, eps: float = 1e-20, sigma: float = 1.0, temperature: float = 0.4):
        """
        temperature 越小 => softmax 越尖锐 => fitness 差异放大越强
        temperature 越大 => 越平滑
        """
        x_t = self.x.long()  # (N, D)
        ...
        # 为了数值稳定，先中心化；temperature 控制放大强度
        # temperature -> 0  越“挑最优”
        # temperature -> inf 越接近均匀
        ...
        # 用 log 累加避免下溢：log_kernel[i,j] = Σ_d log qbar[x_jd, x_id]
        ...
        # p_t = torch.einsum("ij,jdk->idk", W, onehot).clamp_min(eps)  # (N, D, K)
        # p_t = p_t / (p_t.sum(dim=-1, keepdim=True) + eps)

        # ---------- 5) 反推 p0^{(i)}: p_t = p0 @ qbar_t ----------
        # qbar_inv = torch.linalg.pinv(qbar_t)  # (K, K)
        # p0 = torch.matmul(p_t, qbar_inv).clamp_min(0.0) + eps
        # p0 = p0 / (p0.sum(dim=-1, keepdim=True) + eps)

        # ---------- 6) 逐 token 采样得到 x0_hat (N,D) ----------
...
class BayesianGenerator:
    def __init__(self,x,fitness,q,qbar,t):
        self.x = x
        self.fitness = fitness
        self.t=t
        self.q = q
        self.qbar = qbar
        self.estimator = BayesianEstimator(self.x,self.fitness,self.q,self.qbar,t=self.t)

    def generate(self):
        x0_est = self.estimator()
