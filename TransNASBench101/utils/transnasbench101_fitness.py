...
def trans_macro_arch_to_str(arch_matrix: torch.Tensor) -> str:
    arch_str = ""
    mapping = [4, 5, 6, 6]
    for i in range(mapping[arch_matrix[0].item()]):
        arch_str = arch_str + str(arch_matrix[i+1].item()+1)
...
def trans_micro_arch_to_str(arch_matrix: torch.Tensor) -> str:
    ...
    return (
        "64-41414-"
        + str(arch_matrix[0].item())
        + "_"
        + str(arch_matrix[1].item())
        + str(arch_matrix[2].item())
        + "_"
        + str(arch_matrix[3].item())
        + str(arch_matrix[4].item())
        + str(arch_matrix[5].item())
    )
...
def macro_neural_predictor(operation_matrix, api, task):
    ...
    operation_matrix = operation_matrix.view(population, 7)
    acc_list = []
...
def micro_neural_predictor(operation_matrix, api, task):
    population = operation_matrix.shape[0]
    operation_matrix = operation_matrix.view(population, 6)
    acc_list = []
    for i in range(operation_matrix.shape[0]):
        try:
