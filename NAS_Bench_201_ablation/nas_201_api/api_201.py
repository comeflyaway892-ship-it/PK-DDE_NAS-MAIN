...
############################################################################################
# NAS_Bench_201: Extending the Scope of Reproducible Neural Architecture Search, ICLR 2020 #
############################################################################################
# The history of benchmark files:
# [2020.02.25] NAS_Bench_201-v1_0-e61699.pth : 6219 architectures are trained once, 1621 architectures are trained twice, 7785 architectures are trained three times. `LESS` only supports CIFAR10-VALID.
# [2020.03.16] NAS_Bench_201-v1_1-096897.pth : 2225 architectures are trained once, 5439 archiitectures are trained twice, 7961 architectures are trained three times on all training sets. For the hyper-parameters with the total epochs of 12, each model is trained on CIFAR-10, CIFAR-100, ImageNet16-120 once, and is trained on CIFAR-10-VALID twice.
#
...
ALL_BENCHMARK_FILES = ['NAS_Bench_201-v1_0-e61699.pth', 'NAS_Bench_201-v1_1-096897.pth']
ALL_ARCHIVE_DIRS = ['NAS_Bench_201-v1_1-archive']
...
"""
This is the class for the API of NAS_Bench_201.
"""
class NASBench201API(NASBenchMetaAPI):
  ...
  def __init__(self, file_path_or_dict: Optional[Union[Text, Dict]]=None,
    ...
    if file_path_or_dict is None:
      ...
      print ('Try to use the default NAS_Bench_201 path from {:}.'.format(file_path_or_dict))
    if isinstance(file_path_or_dict, str) or isinstance(file_path_or_dict, Path):
      file_path_or_dict = str(file_path_or_dict)
      if verbose: print('try to create the NAS_Bench_201 api from {:}'.format(file_path_or_dict))
  ...
  def str2matrix(arch_str: Text,
    ...
    """
    This func shows how to convert the string-based architecture encoding to the encoding strategy in NAS_Bench_101.

    :param
      arch_str: the input is a string indicates the architecture topology, such as
                    |nor_conv_1x1~0|+|none~0|none~1|+|none~0|none~1|skip_connect~2|
      search_space: a list of operation string, the default list is the search space for NAS_Bench_201
    ...
    :usage
      ...
      In NAS_Bench_201 search space, 0-th-op is 'none', 1-th-op is 'skip_connect',
