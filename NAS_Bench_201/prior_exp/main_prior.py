...
NAS201_ROOT = os.path.dirname(PRIOR_EXP_ROOT)
REPO_ROOT = os.path.dirname(NAS201_ROOT)
for path in (REPO_ROOT, NAS201_ROOT):
    if path in sys.path:
        sys.path.remove(path)
    sys.path.insert(0, path)
os.chdir(NAS201_ROOT)
...
def main(args):
    ...
    assert dataset in dataset_name, f"ERROR: invalid dataset {args.dataset}"
    target = dataset_name[dataset]
    kwargs = {
    ...
    if dataset in ("cifar10", "cifar100", "imagenet"):
        exp_with_fixed_seed_in_nb201_prior(target, **kwargs)
    else:
        exp_with_fixed_seed_in_meta_predictor_prior(target, **kwargs)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="NAS-Bench-201 prior experiment")
    parser.add_argument("--dataset", type=str, default="cifar10", help="cifar10, cifar100, imagenet, aircraft, pets")
    parser.add_argument(
        "--prior-type",
        type=str,
        default="structural",
