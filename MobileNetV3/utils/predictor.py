...
def d3pm_step(xt, x0, q, qbar, t, eps=1e-20):
    ...
    if q.dim() == 3:
        ...
        q_t = q[t - 1]
        qbar_tm1 = qbar[t - 1]
        like = q_t[:, xt_flat].transpose(0, 1).contiguous()
        prior = qbar_tm1[x0_flat]
        post = like * prior
        post = post / post.sum(dim=-1, keepdim=True).clamp_min(eps)
        return torch.multinomial(post, num_samples=1).squeeze(-1).view_as(xt)

    N, D = xt.shape
    K = q.shape[-1]
    q_t = q[t - 1]       # [D, K, K]
    qbar_tm1 = qbar[t - 1]  # [D, K, K]
...
class BayesianEstimator:
    ...
    def estimate(self, eps: float = 1e-20, sigma: float = 1.0, temperature: float = 0.4):
        ...
        qbar_t = self.qbar[t].clamp_min(eps)  # (K, K) or (D, K, K)
        ...
        for d in range(D):
            ...
            qbar_td = qbar_t if qbar_t.dim() == 2 else qbar_t[d]
            M = qbar_td[a.view(1, N), b.view(N, 1)].clamp_min(eps)  # (N, N)
            log_kernel += torch.log(M)

        log_kernel = log_kernel / D

        # 不能直接 kernel=log_kernel（会有负数权重）
...
class BayesianGenerator:
    ...
    def generate(self):
        ...
        if os.environ.get("DDE_NAG_DEBUG_TENSORS", os.environ.get("EDNAG_DEBUG_TENSORS")) == "1":
