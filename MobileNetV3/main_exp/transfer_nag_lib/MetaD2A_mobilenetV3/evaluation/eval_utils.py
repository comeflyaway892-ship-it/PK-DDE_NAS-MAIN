...
def get_dataset(data_name, batch_size, data_path, num_workers,
	...
	print(f"==> get_dataset: name={data_name}, path={data_path}, workers={num_workers}", flush=True)
	if data_name == 'cifar100':
		print("==> loading CIFAR100; torchvision may extract/download if files are missing", flush=True)
		...
		print("==> loading CIFAR10; torchvision may extract/download if files are missing", flush=True)
		...
		if not os.path.exists(data_path):
			raise FileNotFoundError(
				f"Aircraft dataset not found at {data_path}. "
				"Prepare fgvc-aircraft-2013b locally before MobileNetV3 retraining."
			)
		...
		valid_data = FGVCAircraft(data_path, class_type='variant', split='test',
		                          transform=valid_transform, download=False)
	elif data_name.startswith('pets'):
		print("==> loading pets dataset", flush=True)
