...
from all_path import OFA_SUPERNET_CKPT_PATH
...
def set_architecture(n_cls, evaluator, drop_path, drop, img_size, n_gpus, device, save_path, model_str):
	...
	profile_net = copy.deepcopy(net).eval()
	with torch.no_grad():
		flops = profile_macs(profile_net, inputs) / 1e6
	...
	if n_gpus > 1 and torch.cuda.device_count() > 1:
		print("Let's use", min(n_gpus, torch.cuda.device_count()), "GPUs!")
...
def train_single_model(save_path, workers, datasets, xpaths, splits, use_less,
	...
	print(to_save_name)
	# args = get_parse()
	print("==> build OFA evaluator", flush=True)
	num_gen_arch = None
	evaluator = OFAEvaluator(num_gen_arch, img_size, drop_path,
		model_path=OFA_SUPERNET_CKPT_PATH)
	print("==> OFA evaluator ready", flush=True)

	print(f"==> build dataset {datasets} from {xpaths}", flush=True)
	train_queue, valid_queue, n_cls = get_dataset(datasets, batch_size, 
        xpaths, workers, img_size, autoaugment, cutout, cutout_length)
	print(f"==> dataset ready: train_batches={len(train_queue)}, valid_batches={len(valid_queue)}, classes={n_cls}", flush=True)
	print("==> build active subnet", flush=True)
	...
	print("==> active subnet ready", flush=True)
	...
	checkpoint = {
		...
		'training_config': {
			...
			'cutout': bool(cutout),
			'cutout_length': int(cutout_length),
			'autoaugment': bool(autoaugment),
			'drop': float(drop),
	...
	_atomic_torch_save(checkpoint, to_save_name)
	print(f'final checkpoint saved to {to_save_name}')
